...
// ===== DETAIL MODAL =====
const CourtDetailModal = ({ court, onClose }) => {
    const [activeImg, setActiveImg] = useState(0);

    if (!court) return null;

    const images = court.imageUrl
        ? court.imageUrl.split(",").map((s) => s.trim()).filter(Boolean)
        : [];
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div>
                        <h3 className="text-base font-semibold text-gray-800">{court.name}</h3>
                        <p className="text-xs text-gray-400 mt-0.5">{court.address}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6">
                    ...
                    {images.length > 0 ? (
                        <div className="mb-5">
                            <div className="w-full h-64 rounded-xl overflow-hidden bg-gray-50 mb-2">
                                <img
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    {images.map((img, i) => (
                                        <button
                                            ...
                                            className={`w-14 h-14 rounded-lg overflow-hidden border-2 transition-colors ${activeImg === i ? "border-violet-500" : "border-gray-200"}`}
                                        >
                        ...
                        <div className="w-full h-40 rounded-xl bg-gray-50 flex flex-col items-center justify-center mb-5 text-gray-300">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                        <div className="bg-gray-50 rounded-xl p-3">
                            <p className="text-xs text-gray-400 mb-0.5">Chủ sân</p>
                            <p className="font-medium text-gray-700">{court.ownerName || "—"}</p>
                        </div>
                        <div className="bg-gray-50 rounded-xl p-3">
                            <p className="text-xs text-gray-400 mb-0.5">Danh mục</p>
                            <p className="font-medium text-violet-600">{court.categoryName || "—"}</p>
                        </div>
                        <div className="bg-gray-50 rounded-xl p-3">
                            <p className="text-xs text-gray-400 mb-0.5">Commission</p>
                            <p className="font-medium text-gray-700">{court.commissionRate ?? 0}%</p>
                        </div>
                        <div className="bg-gray-50 rounded-xl p-3">
                            <p className="text-xs text-gray-400 mb-0.5">Trạng thái</p>
                            {court.status === "PENDING" && (
                                <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-600">
                                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block" /> Chờ duyệt
                                </span>
                            )}
                            {court.status === "ACTIVE" && (
                        ...
                        {court.description && (
                            <div className="col-span-2 bg-gray-50 rounded-xl p-3">
                                ...
                                <p className="text-gray-700 text-xs leading-relaxed">{court.description}</p>
                            </div>
                        )}
                    </div>
                </div>
                                ...
                                <tbody>
                                    {currentCourts.length === 0 ? (
                                        ...
                                        currentCourts.map((c, idx) => (
                                            ...
                                            >
                                                {/* Sân — bấm tên để xem chi tiết */}
                                                <td className="px-4 py-3 border-r border-gray-200">
                                                    <button
                                                        onClick={() => setSelectedCourt(c)}
                                                        className="text-left group"
                                                    >
                                                        <p className="font-medium text-violet-600 group-hover:underline leading-tight">
                                                            {c.name}
                                                        </p>
                                                        {c.description && (
